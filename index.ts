...
interface Attribute { trait_type: string; value: string | boolean | number; }
...
// 🧬 DNA PARSER (Transforms Raw String to Structured Marketplace Attributes)
...
// 👁️ DUAL-HEMISPHERE ENGINE: NETWORK INTERROGATION (WITH FAILSAFE GET FALLBACK)
async function interrogateServerForMediaType(url: string | null | undefined): Promise<'json' | 'image' | 'video' | 'none'> {
  ...
  try {
    ...
    if (!response.ok && (response.status === 405 || response.status === 403)) {
      response = await fetch(url, { method: 'GET', headers: { 'Range': 'bytes=0-512' }, signal: controller.signal });
    }
...
// ============================================================================
// 🛡️ CORE EXECUTIONS: RPC FAILOVER & JSON CONCURRENT RACING
// ============================================================================
async function executeContractReadWithRetry(tokenId: bigint) {
  ...
  for (const rpcUrl of RPC_ENDPOINTS) {
    try {
      ...
      const timeoutPromise = new Promise<never>((_, rej) => setTimeout(() => rej(new Error('RPC Timeout')), 2500));
...
app.get('/api/metadata/:tokenId', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    if (!/^\d+$/.test(tokenId)) { res.status(200).json(FALLBACK_METADATA); return; }
    parsedTokenId = BigInt(tokenId);
    if (parsedTokenId < 1n || parsedTokenId > 333n) { res.status(200).json(FALLBACK_METADATA); return; }
  } catch {
    res.status(200).json(FALLBACK_METADATA); return;
  }

  try {
    const deedData = await executeContractReadWithRetry(parsedTokenId);
    
    if (!deedData.active) { res.status(200).json(FALLBACK_METADATA); return; }
    ...
    if (!deedData.sanctified) {
      ...
      // 👁️ BRANCH TRUE (Sanctified / God Mode Update Identity)
